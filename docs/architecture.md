...
│   └── ssh.chrispicloud.dev    → ssh://localhost:22  (SSHD)         │
│                                                                      │
│   ┌─────────────────────────────────────────────────────────────┐   │
│   │                     K3s Cluster                              │   │
│   │                                                              │   │
...
1. **GitOps-first** — All deployments are driven by git commits. Flux CD watches repos and applies changes automatically. No manual `kubectl apply`.

2. **Infrastructure independence** — `cloudflared` runs as a systemd service outside K3s, so remote access survives cluster failures.

3. **Namespace isolation** — Each application gets its own namespace with dedicated secrets and resources. Dev and prod are fully separated.

4. **ARM64-native** — All container images are built for `linux/arm64` via GitHub Actions cross-compilation (QEMU + buildx).

5. **No open ports** — The Pi has no inbound ports open. All external access is through the outbound Cloudflare Tunnel.

6. **Iterative evolution** — Start as a monolith, extract services as complexity grows. Platform services (auth, monitoring) are added incrementally.
